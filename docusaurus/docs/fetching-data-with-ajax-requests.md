...
Inferno doesn't prescribe a specific approach to data fetching, but people commonly use either a library like [axios](https://github.com/axios/axios) or the [`fetch()` API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) provided by the browser.
...
For example, support in Internet Explorer requires a [polyfill](https://github.com/facebook/create-inferno-app/blob/main/packages/inferno-app-polyfill/README.md).

You can learn more about making AJAX requests from Inferno components in [the FAQ entry on the Inferno website](https://infernojs.org/docs/faq-ajax.html).
